...
**** Fix formatting of title and first header in daily note
If we don't hook this function, then the first header is right up against the title. We use this to add an extra newline character underneath it to give the first header some room.
#+begin_src emacs-lisp
...
**** org add todo keywords
This is needed for the following block the sets up todo rollovers for dailies.
#+begin_src emacs-lisp
...
**** Set up to-do rollover for dailies
#+begin_src emacs-lisp
